...
describe('When login is not ok, and email is blank.', () => {
  ...
  it('Return status code 401', async () => {
    chaiHttpResponse = await chai
      .request(app).post('/login').send({ email: EMAIL_BLANK, password: PASSWORD_TYPE2_TEST });
    expect(chaiHttpResponse.status).to.be.equal(401);
  });
});
...
describe('LOGIN Validate Tests', async () => {
  let chaiHttpResponse: Response;
  chaiHttpResponse = await chai.request(app).post('/login').send({ email: EMAIL_ADMIN, password: PASSWORD_CORRECT });
  ...
  describe('When token is incorrect', async () => {
    ...
    it('Return status code 401', async () => {
      chaiHttpResponse = await chai
        .request(app).get('/login/validate').set({ authorization: 'notatoken' })
      expect(chaiHttpResponse.status).to.be.equal(401);
    });
  });
